...
(async () => {
    const browser = await puppeteer_1.default.launch({
        ...
        executablePath: "C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe", // Path to your installed Chrome browser
    ...
    const AmazonproductInfo = await amazonPage.evaluate(() => {
        const products = Array.from(document.querySelectorAll('[data-component-type="s-search-result"]')).slice(0, 10);
        return products.map((product) => {
            const sponsoredElement = product.querySelector(".puis-sponsored-label-text");
            if (sponsoredElement) {
                return null; // Skip sponsored products
            }
            const titleElement = product.querySelector("span.a-size-medium.a-color-base.a-text-normal");
    ...
    const EbayproductInfo = await ebayPage.evaluate(() => {
        const products = Array.from(document.querySelectorAll(".s-item")).slice(1, 11);
        return products.map((product) => {
